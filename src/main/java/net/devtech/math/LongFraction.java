...
public final class LongFraction implements Fraction {
	...
	static {
		for (int i = 0; i < CACHE.length; i++) {
			CACHE[i] = new LongFraction(i / 16, (i % 16) + 1);
		}
	}
	...
	public Fraction add(Fraction amount) {
		if (amount instanceof LongFraction) {
			...
			// naive method
			if (canMultiply(fraction.denominator, this.denominator) && canMultiply(fraction.numerator,
			                                                                       this.denominator) && canMultiply(fraction.denominator,
			                                                                                                        this.numerator)) {
